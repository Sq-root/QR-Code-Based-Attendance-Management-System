...
// Random attendee names for simulated scan events
...
export const Dashboard: React.FC = () => {
  ...
  // Trigger Scanner View and simulate success
  const handleOpenScanner = () => {
    setScannerOpen(true);
    setScannerScanning(true);
    setScannerSuccessName('');

    // Simulate scanning/detecting a code after 2.2 seconds
    setTimeout(() => {
      const luckyName = ATTENDEE_NAMES[Math.floor(Math.random() * ATTENDEE_NAMES.length)];
      ...
      // Confirm verification after another 1 second
      setTimeout(() => {
        const initials = luckyName.split(' ').map(n => n[0]).join('').toUpperCase();
        ...
        const newLog: ActivityLog = {
          ...
          name: luckyName,
          initials: initials,
          avatarBg: 'bg-tertiary-fixed-dim/20 text-on-tertiary-container border border-on-tertiary-container/15',
      ...
      <div className="flex-1 flex flex-col md:ml-64 min-w-0">
        ...
        <main className="flex-1 px-margin-mobile md:px-8 py-6 w-full max-w-[1600px] mx-auto space-y-6 animate-fade-in">
          ...
          <MetricGrid 
            ...
            presentToday={logs.filter(l => l.status === 'Success').length + 958} // Dynamic presence count (26 initial + 958 = 984)
            attendanceRate="78.8%"
          ...
          <div className="grid grid-cols-2 gap-4 md:hidden">
            ...
            >
              ...
              <span className="font-sans text-label-md font-bold">Manual Entry</span>
            </button>
          </div>
