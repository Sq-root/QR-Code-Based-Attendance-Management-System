...
import { LayoutDashboard, LogOut, UserPlus, Users, X } from 'lucide-react';
        ...
        <nav className="flex flex-col gap-1">
          {navItems.map(({ to, label, icon: Icon, end }) => (
            <NavLink
              ...
              onClick={closeMobileSidebar}
              className={({ isActive }) =>
                `w-full flex items-center gap-3 rounded-lg px-4 py-2.5 font-sans text-body-sm font-semibold transition-all duration-150 relative cursor-pointer ${
